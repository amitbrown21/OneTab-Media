...
// Lightweight logger to help diagnose options init issues
...
async function initializeOptionsPage() {
  logOptions('init: start');
  // First migrate from local storage if needed (so we don't overwrite with defaults)
  await migrateFromLocalStorage();
  logOptions('init: after migrate');

  // Then ensure defaults exist only for missing keys
  await ensureDefaultsSet();
  logOptions('init: after ensureDefaultsSet');
  
  // Load current settings
  await loadSettings();
  logOptions('init: after loadSettings');
  ...
  logOptions('init: theme controls ready');
}
...
async function migrateFromLocalStorage() {
  try {
    ...
    logOptions('migrate: hasSyncData', hasSyncData);
    ...
    if (!hasSyncData) {
      ...
      logOptions('migrate: hasLocalData', hasLocalData);
      
      if (hasLocalData) {
        logOptions('migrate: moving local->sync');
        await syncSet(localData);
        showStatus('Settings migrated to sync storage for cross-device compatibility', 'success', 4000);
      }
    }
  } catch (error) {
    console.warn('[UME Options] migrate error:', error);
  }
}
...
async function ensureDefaultsSet() {
  try {
    ...
    logOptions('ensureDefaultsSet: current keys', Object.keys(result));
    ...
    if (isFreshInstall && !alreadyInitialized) {
      logOptions('ensureDefaultsSet: fresh install, writing defaults');
      ...
      if (Object.keys(updates).length > 0) {
        logOptions('ensureDefaultsSet: setting missing defaults', updates);
    ...
    console.error('[UME Options] ensureDefaultsSet error:', error);
...
async function cleanupDeprecatedSettings() {
  try {
    ...
    await syncRemove(deprecatedKeys);
    await localRemove(deprecatedKeys);
  } catch (error) {
...
async function loadSettings() {
  try {
    ...
    logOptions('loadSettings: loaded', result ? Object.keys(result).length : 0);
    ...
    console.error('[UME Options] loadSettings error:', error);
